...
const ShopContextProvider = (props) => {
    ...
    useEffect(() => {
        fetch('https://e-commerce-12i-rc.onrender.com/allproducts')
        .then((response) => response.json())
        .then((data) => setAll_Products(data))


        if(localStorage.getItem('token')){
            fetch('https://e-commerce-12i-rc.onrender.com/getcart', {
                method: 'POST',
                headers: {
    ...
    const addToCart = (itemId) => {
        ...
        if(localStorage.getItem('token')){
            fetch('https://e-commerce-12i-rc.onrender.com/addtocart', {
                method: 'POST',
                headers: {
                    Accept: 'application/form-data',
                    'token':`${localStorage.getItem('token')}`,
    ...
    const removeFromCart = (itemId) => {
        setCartItems((prev) => ({...prev , [itemId]: prev[itemId] - 1}))
        if(localStorage.getItem('token')){
            fetch('https://e-commerce-12i-rc.onrender.com/removefromcart', {
                method: 'POST',
                headers: {
                    Accept: 'application/form-data',
                    'token':`${localStorage.getItem('token')}`,
